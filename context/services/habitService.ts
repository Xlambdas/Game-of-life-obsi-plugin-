...
export default class HabitService {
	...
	updateHistory(habit: Habit): Habit {
		...
		if (recurrence.type === 'interval') {
			...
			const rebuiltHistory = validatableDates.map(date =>
				habit.streak.history.find(h => DateHelper.toDateString(h.date) === date)
				?? { date, success: false }
			);
	...
	async updateHabitCompletion(
		...
		const newEntry = { date: dateStr, success: completed };
		...
		let { current, best, freezeAvailable, freezeUsedDates } =
			this.computeStreaks(habit, history, recurrence);
