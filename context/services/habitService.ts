...
export class HabitService {
	...
	*/
	...
	refreshHabits(habit: Habit): Habit {
		...
		// }
		console.log(
			`[refreshHabits] Habit ${habit.id} refreshed: isCompletedToday=${habit.streak.isCompletedToday}, lastCompletedDate=${habit.streak.lastCompletedDate?.toDateString()}, nextDate=${habit.streak.nextDate?.toDateString()}`
		);
...
export class HabitService_test {
	private appContext: AppContextService;
	 ...
	 * @param date The date to test (default: today).
	 */
	isCompleted(habit: Habit, date: Date = new Date()): boolean {
		const normalizeDate = (d: Date | string) => new Date(d).toDateString();
		const dateStr = normalizeDate(date);
		...
		return habit.streak.history.some(
			entry => normalizeDate(entry.date) === dateStr && entry.success
		);
	}
	...
	async updateHabitCompletion(
		...
		if (completed && existingEntry?.success) {
			throw new Error(
				`Habit ${habit.id} already completed for ${dateStr}. No further action possible.`
			);
		}
		if (!completed && !existingEntry) {
			console.warn(
		...
		// calculate key dates
		const todayStr = new Date().toDateString();
		const isCompletedToday = this.isCompleted({ ...habit, streak: { ...habit.streak, history } });
	...
	private calculateNextDate(
		...
		if (!fromDate || isNaN(fromDate.getTime())) {
			return fallbackCreatedAt ? new Date(fallbackCreatedAt) : new Date(0);
		}
		if (fromDate.getTime() === 0) {
			return fallbackCreatedAt ? new Date(fallbackCreatedAt) : new Date(0);
		}
	...
	refreshHabits(habit: Habit): Habit {
		...
		if (lastCompleted) {
			const lastDate = new Date(lastCompleted.date);
			const { interval, unit } = habit.recurrence;
			nextDate = this.calculateNextDate(lastDate, habit.recurrence);
		}
